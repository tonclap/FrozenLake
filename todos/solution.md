...
> **СТАТУС: закрыто 2026-08-09. Достигнуто 100% (3828/3828).**
> Модель — `models/10000_curriculum_ep5200.pth`, метод — `curriculum_trainer.py`.
> Сработал не план из этого документа, а устранение рассогласования распределений:
> обучение сэмплировало карты через вероятность льда `p`, а оценка шла равномерно по
> всему перечислению. Разбор — в README, раздел «Solving all maps».
>
> **Две поправки к диагнозу ниже:**
> 1. Признак «кратчайший путь = 6» ничего не различает: путь равен 6 у **всех 3828**
>    валидных карт, а не только у провальных. Реальный предиктор — плотность дыр:
>    провалов 0% при ≤3 дырах, 1.2% при 7, 4.7% при 8 и 10% при 9.
> 2. Этапы 1.1 (инъекция трудных карт) и 1.2 (BFS вместо Манхэттена в канале 2) были
>    реализованы в коммите `75a343f` и до 100% не довели: с ними потолок составил
>    99.53%. Инъекция по `p` бьёт мимо, потому что задаёт плотность дыр лишь в
>    среднем и всё равно не покрывает нужные раскладки адресно.
